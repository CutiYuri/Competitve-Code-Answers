...
let rec try_exception (env_so_far: subst) (free_vars: string list) 
   ...
   |[] -> let result = eval f env_so_far in  if result = false then 
    raise (FoundCounterExample env_so_far) else ()
   |x::xs -> try_exception ((x,true)::env_so_far) xs f;
             try_exception ((x,false)::env_so_far) xs f
